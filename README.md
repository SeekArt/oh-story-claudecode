...
**`.active-book`：** 项目根目录的文本文件，内容是当前活跃书目的**相对路径**（如 `长篇/我的小说`、`书本/20260301/道诡异仙`），hook 和写作 skill 据此定位当前项目。路径不限深度，只要能指向书目目录即可。

### 多书结构

项目支持同时管理多本书，书目录不限前缀、不限深度。只要目录下包含 `追踪/` 或 `设定/` 子目录，就会自动识别为一本书：
...
│   │   └── 择日飞升/
│   └── ...
├── .active-book             # 当前活跃：书本/20260301/宿命之环
└── 拆文库/                  # 共享分析资产
    └── {对标书名}/
```

切换书目时运行 `/story` → "切书"或"列出我的书"，从扫描结果中选择即可。
