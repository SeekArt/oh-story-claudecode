...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   **知识边界提取**（状态筛选的一部分）：对每个出场角色，从角色设定文件中分离三类信息——已知、未知、不应知道。来源映射见 `references/writing-constraint-card.md` → 知识边界处理流程 → Step B。如果角色设定未显式列出知识范围，从该角色参与过的正文章节（在追踪/时间线.md 中标记的章节）推断已知信息，从未出场但被提及的事件推断未知信息。提取结果写入意图确认的连续性约束部分（Step 2.4）。

   **引用完整性 grep**（状态筛选的一部分，必做）：用 grep 做引用存在性验证，排查"你不知道你不知道"的遗漏。在加载角色设定和道具清单后，对本章涉及的关键实体跑以下检查：

   ```bash
   # 角色→物品：角色持有的物品是否在道具清单里有记录
   grep -c "黑钥匙" 设定/道具清单.md  || echo "[完整性警告] 黑钥匙未在道具清单中记录"

   # 物品→角色：物品持有者是否在角色列表里存在
   grep -c "林月" 设定/角色/林月.md  || echo "[完整性警告] 道具持有者林月没有角色设定文件"

   # 角色→关系：角色之间的关系是否在关系文件中有记录
   grep -c "林月" 设定/关系.md  || echo "[完整性警告] 林月在关系.md 中无记录"

   # 正文→设定：本章涉及的关键地点/物品/能力是否在对应的设定文件中有条目
   grep -c "杭州" 设定/地点/杭州.md  || echo "[完整性警告] 杭州未在设定中有独立文件"
   ```
   警告命中时，在意图确认中标记引用缺失，但不阻塞写作。警告意味着"你正在写一个没有设定文件的实体"，需要特别注意不要在正文中建立该实体与已有设定矛盾的事实。
   - **2.3 文风召回**：
   ...
   - **2.4 意图确认**：从细纲「目标情绪」字段确认本章情绪目标，综合状态筛选结果 + 文风召回输出 + **约束卡（过一遍 `references/writing-constraint-card.md` 的 8 维，提取 2-3 条本章最关键的连续性约束）**，用一句话写本章意图（情绪+节奏+文风指令+连续性约束）。文风指令例：「标点照文风里的停顿节奏、对话潜台词用问非所答、情绪交替参考第 K 章爽点铺放比。」连续性约束例：「注意苏念仍不知道林月的真实身份，对话中不要越界。」
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则强制扩充）** → 检查钩子/爽点 → 禁用词扫描
   - 每章写完后**立即更新**以下文件（**追加式更新，不覆盖已有历史**）：

     格式规则：
     - `追踪/角色状态.md`：保留"变更历史"和"回退记录"两个追加区域，不允许整段替换。写前确认历史表格的最后一行，以追加行加入新记录而非重写整个表格。
       ...
       | 日期 | 章节 | 实体 | 旧值 → 新值 | 原因 |
       |------|------|------|-------------|------|
       | {日期} | 第{N}章 | 林月_位置 | 北京 → 杭州 | 南下寻药 |
       | {日期} | 第{N}章 | 林月_持有 | 无 → 黑钥匙 | 遗迹获得 |

       # 回退记录（retcon）
       | 日期 | 回退内容 | 旧值 → 新值 | 原因 |
       |------|----------|-------------|------|
       ```
     - `追踪/伏笔.md`：追加新增/状态变更的记录行，不覆盖已有伏笔列表。如回收伏笔，在原有行上追加 `→ 已回收` 标记而非删除该行。
     - `追踪/时间线.md`：按时间顺序追加新事件，不修改已有事件记录。
     - `追踪/上下文.md`：只更新进度元数据（当前位置+已写字数+本次变更摘要），不记录详细角色状态/伏笔内容。**这里允许覆盖**（元数据是最新快照）。
...
5. **反向验证（正文→设定）**：对本章正文中出现的关键角色名/物品名/地名，反向 grep 检查是否在对应设定文件中有记录。只标记警告，不阻塞：

   ```bash
   grep -of <(grep -oP '林月|黑钥匙|杭州' 正文/第{N}章_*.md) 设定/角色/ 设定/道具清单.md 设定/地点/ | sort
   ```
   未命中的实体标记为"正文中出现了但没有设定文件记录的实体"，建议补建。
6. **持有链连续性 grep**：对本章涉及的每个关键物品，检查其持有者变化是否有过程交代：

   ```bash
   # 从道具清单提取本章物品的当前持有者
   grep -A2 "黑钥匙" 设定/道具清单.md | grep "持有"
   # 对比上一章正文中的持有状态
   grep "黑钥匙" 正文/第{N-1}章_*.md
   grep "黑钥匙" 正文/第{N}章_*.md
   ```
   如果持有者在本章发生了变化且没有转手过程描写，标记为连续性风险。
7. **标点确定性收尾**：对本批所有新写正文文件运行 `node scripts/normalize-punctuation.js 正文/第XXX章_*.md`（写模式，默认 `--quote-mode keep`），确定性去掉叙述里的破折号 `——`/`—`、双连字符 `--` 和独立行 `---`，避免长篇写到上百章后横线累积。脚本只改这些标点：对话被打断的 `——`、数字区间 `100——200` 原样保留，盐言「」不受影响。**由谁运行**：narrative-writer agent 不运行本脚本；主会话在 agent 写完返回后、针对实际落盘的正文文件路径运行（文件名是写完才确定的 `第XXX章_章名.md`）。
