...
在进入 Phase 2 之前，检测可用的执行方式：

**检查项**：
- `.story-deployed` 是否存在
- `.kilo/agent/chapter-extractor.md` 或 `.claude/agents/chapter-extractor.md` 是否存在（项目 agents）
- 当前环境是否支持子代理调用能力（如 Task 工具）

**执行方式判定**：
1. **项目 agents 可用** → Phase 2 使用并行 chapter-extractor agents
2. **环境子代理可用** → Phase 2 使用环境子代理并行执行
3. **都不可用** → Phase 2 降级为串行处理

**提示用户**（仅在降级时）：

> 「检测到当前项目尚未部署写作基础设施，且环境不支持并行子代理。Phase 2 逐章摘要将使用串行处理，速度较慢，但产物完整。如需加速，可运行 `/story-setup` 部署 agent。」

给用户选择：
1. **先去 setup**：运行 `/story-setup` 部署，完成后重新 `/story-import`
2. **继续导入**：接受串行处理
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色）。每章3-40情节点（密度150-200字/个，按字数动态调节）。角色过滤（龙套不提取、别名归类）。**并行执行模式：优先使用项目 chapter-extractor agent，否则使用环境子代理，最后降级串行**。**计数验证：摘要数 == 章节数**。 | 所有章节处理完成 |
...
沿用 story-long-analyze 的分块策略（Stage 2 优先使用 chapter-extractor agent 并行，agent 不可用时使用环境子代理或串行处理，其他阶段按以下策略分块）：
...
> 「书 `{书名}` 的导入位置：
> - **项目根目录**：`{书名}/`（默认，最简单）
> - **按篇幅分类**：`长篇/{书名}/` 或 `短篇/{标题}/`（传统方式）
> - **指定分组**：如 `书本/20260301/{书名}/`、`系列/XX宇宙/{书名}/`
> - **其他**：用户自定义路径」

取用户选择（或确认默认），记为 `{目标父路径}`。后续所有 `{书名}/` 或 `{短篇标题}/` 均替换为 `{目标父路径}/{书名}/`。
