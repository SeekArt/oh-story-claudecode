...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色）。角色过滤（龙套不提取、别名归类）。每章10-40情节点（密度150-200字/个，按字数动态调节；公式低于10时仍按硬下限10拆足关键步骤）。**并行执行模式：优先使用项目 chapter-extractor agent，否则使用环境子代理能力，最后降级串行处理**。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
Stage 2 使用 chapter-extractor agent 并行处理每章。根据环境自动选择执行方式。

### 执行方式选择（优先级递减）

**方式 A - 项目部署的 Agent**：
- 检测 `.kilo/agent/chapter-extractor.md` 或 `.claude/agents/chapter-extractor.md`
- 使用 Agent 工具调用：`Agent(subagent_type: "chapter-extractor", prompt: "...")`
- 模型配置使用 agent 定义的 `model` 字段

**方式 B - 环境子代理能力**：
- 项目 agents 不可用，但当前环境支持子代理调用（如 Task 工具）
- 使用环境子代理执行 chapter-extractor 任务，将能力描述直接写在 prompt 中
- 根据环境能力选择并行（同时启动多个任务）或串行执行

**方式 C - 串行处理**：
- 既无项目 agents，也无环境子代理能力
- 由主线程按 chapter-extractor 方法论逐章处理
- 结果格式与 agent 输出完全相同
...
你是章节提取器，负责从小说章节中提取结构化信息。

[完整的 chapter-extractor 能力描述，包括输出格式要求]
...
- 每个执行单元返回 markdown 格式的提取结果
- 主线程将输出写入 `章节/第{N}章_摘要.md`
- 收集所有出场人物表，供 Stage 3 合并使用

### 失败处理 + 质量升级重试

**两类失败**：
1. **执行失败**（crash / 超时 / 空输出）→ 同级别重试 1 次
2. **质量失败**（输出落盘后跑「质量检查」9 条自检，任一不达标）→ **升级到高级别模型重试 1 次**

**最终落盘规则**：
- 首次通过 → 写入 `章节/第{N}章_摘要.md`，`_progress.md` 标记 `success`
- 失败 + 同级别 retry 通过 → 同上，备注 `retry_same_level`
- 质量失败 + 高级别 retry 通过 → 同上，备注 `retry_upgraded`
- 高级别 retry 仍失败 → 章节标记 `⚠️ 跳过`，失败原因写入 `_progress.md`「失败记录」表
