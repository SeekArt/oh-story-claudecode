...
**正向替换指南（词汇降维）**：不只停留在"别用什么"，还要知道"该用什么"。详情见 `references/anti-ai-writing.md → 词感强化技巧`。

核心原则：
- **叠词增强**：单音节/抽象 → AA/AABB 式叠词（用力→死死、沉重→重重）
- **口语替代**：书面词→市井词（得到→弄到、引起→弄出、极度→格外）
- **主观注入**：比喻前加主观判断（像→活脱脱）
...
**句式级黑名单**（详见 `references/anti-ai-writing.md → AI 高概率特征黑名单`）：

| 句式模式 | 问题 | 处理 |
|---------|------|------|
| 连续3+排比 | 最明显的AI签名 | 拆到≤2句或改写 |
| "不是...是..." | AI最爱哲理否定 | 禁止连续用，单章≤1次 |
| 过渡词密集 | 然后/但是/因为堆砌 | 每100字"然后"≤2次 |
| "他/她——"式 | 破折号引导心理转折 | 每章破折号≤2次 |
| 抽象结尾 | "不知道在对谁说" | 用动作/感官收尾 |
| 数字重复 | "二十年。二十年。" | 换说法表达 |
| 情绪四步递进 | 事实→重复→否定→升华 | 跳过中间直达核心 |
| 无生命物拟人动词 | "数据告诉我们""时间会证明" | 找到动作的行为人，如无则用被动或省略 |
...
**动作中间帧**：关键动作拆解为更细碎的动作序列，注入力量感。只在关键动作上使用，一场戏≤3处。

| 平淡 | 中间帧 |
|------|--------|
| 她扯下钱袋 | 她一把扯下那个钱袋 |
| 他拿起杯子 | 他伸手握住杯子，指节泛白，端起来 |

**视角越界（POV Bleed）**：角色内心活动无缝融入旁白，不加引号或标记。

- ❌ `他心里疯狂尖叫着完了完了`
- ✅ `该死。这特么要是接不上话，今天就得死在这儿。后背的冷汗瞬间就下来了。`

规则：
- 减少"他心想/他觉得/他在心里"显式标记
- 情绪不贴标签，改用动作、停顿、环境暗示
- 允许旁白中出现角色特有的语言习惯（粗口、口头禅）

**上帝视角悬浮感（Narrator-from-a-distance）**：AI 写作常从高空俯瞰群体，而非置身场景内部视角。
...
处理：将群像虚指改为具体角色的视角观察。如果场景中没有具体角色可以承载观察，用环境细节替代。
- ❌ "没有人知道这天夜里发生了什么。"
- ✅ "巷子里的路灯灭了一盏。"
...
| 场景 | 节奏策略 | 示例 |
|------|---------|------|
| 紧张/对峙/震惊 | 极短句，单字成段 | `他没动。他不能动。不敢动。` |
| 连续动作/快速思考 | 一气呵成较少标点 | 模拟紧绷神经的连贯感 |
| 情绪高潮 | 不正确标点 | 用..、半个引号营造失控 |
| 情绪低谷/沉静 | 句长拉长，信息密度降低 | 配合景物或慢动作 |
| 日常过渡 | 长短交错，自然停顿 | 读起来不卡即可 |

**句长节奏模式**（交替使用）：
- `长-短-短`：长句铺垫→两短句收束。适合揭示事实或高潮释放。
- `短-长-短`：短句设悬念→长句推进→短句落点。适合动作描写。
- 交替不要生硬，一场景一种节奏就够了。
...
**量子对话（30% 非线性）**：约 30% 的对话内容可以偏离直线逻辑。

| 技法 | 说明 | 示例 |
|------|------|------|
| 答非所问 | 回答不直接回应问题 | "你去哪了？""你管我。" |
| 只说半句 | 话不说完，留白 | "你要是敢……" |
| 打断 | 一方没说完被截断 | "你听我解释——""不听。" |
| 潜台词优先 | 字面A，意指-A或B | "你饿不饿？"（≈我错了，别生气了） |

**控制熵（引入真实对话杂质）**：

| 元素 | 作用 | 示例 |
|------|------|------|
| 语气词 | 增加情绪温度 | 啧、呵、哎、嘁、嘶 |
| 停顿 | 表示犹豫或威胁 | "那……你确定？" |
| 废话 | 真实对话有填充词 | "那个吧……其实也不是不行" |
| 无意义微操作 | 边说边做无关动作 | 弹烟灰、碾石子、转笔 |
